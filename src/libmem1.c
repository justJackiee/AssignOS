...
int __free(struct pcb_t *caller, int vmaid, int rgid)
{
  ...
  int ret = enlist_vm_freerg_list(caller->mm, &rgnode);
...
int get_free_vmrg_area(struct pcb_t *caller, int vmaid, int size, struct vm_rg_struct *newrg)
{
  ...
  if(tmp){
    if(tmp->rg_start + size < tmp->rg_end){
      tmp->rg_start = tmp->rg_start + size;
    }
    else{
      struct vm_rg_struct* nexttmp = tmp->rg_next;
      tmp->rg_next = (nexttmp) ? nexttmp->rg_next : NULL;
      tmp->rg_start = (nexttmp) ? nexttmp->rg_start : -1;
      tmp->rg_end = (nexttmp) ? nexttmp->rg_end : -1;
      nexttmp->rg_next = NULL;
      free(nexttmp);
    }
  }
